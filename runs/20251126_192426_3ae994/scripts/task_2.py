...
# Configuration
RUN_ID = "20251126_192426_3ae994"
DB_PATH = Path("Z:\Aastha\market_data_puller\workspace\orchestrator_results.db")


async def task_task_1() -> Dict[str, Any]:
    """
    Task task_1: Retrieve Polymarket prediction data for Bitcoin in 2025
    Agent: polymarket_agent
    Tools: search_polymarket_markets, get_polymarket_history
    """
    task_id = "task_1"
    agent_name = "polymarket_agent"
    
    logger.info(f"Starting task {task_id}: Retrieve Polymarket prediction data for Bitcoin in...")
    logger.info(f"Using tools: search_polymarket_markets, get_polymarket_history")
    ...
    try:
        ...
        tool_params_search_polymarket_markets = {'query': 'bitcoin prediction 2025', 'session_id': '20251126_120000_abc123', 'limit': 5}
        ...
        # Call tool: get_polymarket_history
        logger.info(f"Calling tool: get_polymarket_history")
        tool_func = get_tool("get_polymarket_history")
        if tool_func is None:
            raise Exception(f"Tool not found: get_polymarket_history")
        
        tool_params_get_polymarket_history = {'session_id': '20251126_120000_abc123', 'limit': 5}
        result_get_polymarket_history = tool_func(**tool_params_get_polymarket_history)
        tool_results["get_polymarket_history"] = result_get_polymarket_history
        logger.info(f"Tool get_polymarket_history completed")
