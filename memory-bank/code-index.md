...
### telegram_client.py
Legacy placeholder for a stdlib Telegram Bot API client (deprecated). The
active trading lexicon pipeline no longer uses this module.

### text_normalization.py
Text normalization utilities for trading chat analysis. `normalize_message()`
strips URLs, collapses whitespace, and produces cleaned text plus a token list
for lexicon extraction.

### trading_lexicon.py
Trading lexicon builder that aggregates tokens from Telegram messages into an
in-memory lexicon with frequency stats, ambiguity flags, and example sentences,
and exports it to `workspace/trading_lexicon.json`.
...
### planner_gepa_opt.py
**GEPA optimization harness for Orchestrator Planner**. Offline DSPy-based prompt optimization using heavy-budget GEPA (300 full evaluations, multi-threaded). By default, loads its training queries from `workspace/planner_queries_noisy.txt` (clean + noisy variants, typically generated by `scripts/noise_queries.py`); falls back to `workspace/planner_queries.txt` or an internal 20-query set if those files are missing. Evaluation combines structural metrics (Python-based: task count, dependencies, agent mappings) with LLM-as-judge semantic scoring (coverage, granularity, faithfulness, agent accuracy). Configured for `gpt-5`-class models with robust JSON parsing (handles formatting errors gracefully). Outputs best prompt to `gepa_logs/best_prompt.txt`. Requires `dspy-ai` and `openai` packages, and can optionally log optimisation metrics to Weights & Biases when `wandb` is installed and `WANDB_PROJECT`/`WANDB_RUN_NAME` are configured. See `scripts/GEPA_README.md` for usage guide.
...
### fetch_telegram_trading_chat.py
Legacy placeholder script for Bot API-based fetching (deprecated). Kept only
for historical reference; not used by the current lexicon pipeline.

### build_trading_lexicon.py
Builds the JSON trading lexicon from exported Telegram messages. Reads a JSON
file (e.g. `user_messages.json`), aggregates terms with
