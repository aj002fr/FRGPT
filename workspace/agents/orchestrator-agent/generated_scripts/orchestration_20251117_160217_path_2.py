...
async def task_task_2() -> Dict[str, Any]:
    """
    Task task_2: Fetch the latest predictions on federal interest rate cuts from Polymarket
    Agent: polymarket_agent
    """
    task_id = "task_2"
    agent_name = "polymarket_agent"
    
    logger.info(f"Starting task {task_id}: Fetch the latest predictions on federal interest r...")
    ...
    try:
        ...
        output_path = agent.run(query="fetch the latest predictions on federal interest rate cuts from polymarket", session_id=None, limit=10)
