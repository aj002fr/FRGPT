...
async def task_1() -> Dict[str, Any]:
    """
    Task 1: Find the most recent date when TY price closed between 112.5 and 113
    Agent: market_data_agent
    """
    logger.info(f"Starting task 1: Find the most recent date when TY price closed bet...")
    try:
        agent = MarketDataAgent()
        output_path = agent.run(template="custom", params={'conditions': 'price BETWEEN ? AND ? AND is_valid = 1', 'values': [112.5, 113.0]}, columns=None, limit=1, order_by_column="file_date", order_by_direction="DESC")
