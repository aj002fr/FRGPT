...
# Configuration
RUN_ID = "20251125_185827_01f84a"
DB_PATH = Path("Z:\Aastha\market_data_puller\workspace\orchestrator_results.db")


async def task_task_1() -> Dict[str, Any]:
    """
    Task task_1: Search for relevant Polymarket markets matching the topic of Bitcoin
    Agent: polymarket_agent
    """
    task_id = "task_1"
    agent_name = "polymarket_agent"
    
    logger.info(f"Starting task {task_id}: Search for relevant Polymarket markets matching th...")
    ...
    try:
        ...
        output_path = agent.run(query="search for relevant polymarket markets matching the topic of bitcoin", session_id=None, limit=10)
...
async def task_task_2() -> Dict[str, Any]:
    """
    Task task_2: Retrieve current market data (prices, volumes, liquidity) for the discovered Bitcoin markets
    Agent: polymarket_agent
    """
    task_id = "task_2"
    agent_name = "polymarket_agent"
    ...
    try:
        # Execute agent
        agent = PolymarketAgent()
        output_path = agent.run(query="retrieve current market data (prices, volumes, liquidity) for the discovered bitcoin markets", session_id=None, limit=10)
...
async def task_task_4() -> Dict[str, Any]:
    """
    Task task_4: Analyze repricing or trend evolution to detect consensus shifts or repricing patterns
    Agent: polymarket_agent
    """
    task_id = "task_4"
    agent_name = "polymarket_agent"
    
    logger.info(f"Starting task {task_id}: Analyze repricing or trend evolution to detect con...")
    ...
    try:
        # Execute agent
        agent = PolymarketAgent()
        output_path = agent.run(query="analyze repricing or trend evolution to detect consensus shifts or repricing patterns", session_id=None, limit=10)
